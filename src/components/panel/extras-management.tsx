        ...
        <div className="fixed inset-0 z-40 overflow-y-auto bg-black/45 p-3 sm:p-4" onClick={() => { setIsCreateOpen(false); setDraft(emptyDraft); }}>
          <div className="mx-auto my-6 w-full max-w-2xl rounded-[24px] border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[var(--shadow)] sm:rounded-3xl sm:p-5 md:p-6" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold tracking-[-0.02em] sm:text-xl">Agregar extra</h3>
            ...
            <form className="mt-4 grid max-h-[calc(100vh-180px)] gap-3 overflow-y-auto pr-1 md:grid-cols-3" onSubmit={submitCreate}>
              <ExtraFormFields draft={draft} onChange={setDraft} />
              <div className="md:col-span-3 flex flex-wrap gap-2">
                <button type="submit" className="rounded-full bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--brand-dark)]">Guardar extra</button>
